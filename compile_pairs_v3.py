...
for folder_name in os.listdir(main_directory):
    ...
    if os.path.isdir(folder_path):
        ...
        if len(scala_code) > 0 and verilog_code:
            scala_comb = ""
            for name, code in zip(scala_names, scala_code):
                scala_comb += f"File {name}:\n{code}\n"
            formatted_pair = {
                "prompt": f"Question:Generate the Verilog code corresponding to the following Chisel files.\n{scala_comb}\nAnswer:",
                "chosen": verilog_code
            }
